    ...
    try {
        ...
        if (!hasPremium) {
            embed = new EmbedBuilder()
                .setTitle(`📊 ${interaction.user.username} のステータス (FREE版)`)
                .setColor(0x00FF00)
                .addFields(
                    { name: '💰 所持残高', value: `**${user.points}**`, inline: true },
                    { name: '🔥 連続ログイン日数', value: `**${user.streakCount}日**`, inline: true }
                )
            ...
            embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '💰 所持残高', value: `**${user.points}**`, inline: false }
                )
                .setThumbnail(interaction.user.displayAvatarURL());
        }
